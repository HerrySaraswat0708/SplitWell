...
router.get('/monthly', auth, (req, res) => {
  const rows = db.prepare(`
    SELECT strftime('%Y-%m', e.date) as month, SUM(es.amount) as amount
    FROM expense_splits es
    JOIN expenses e ON es.expense_id = e.id
    JOIN group_members gm ON e.group_id = gm.group_id AND gm.user_id = ?
    WHERE es.user_id = ? AND e.date >= date('now', '-6 months')
...
router.get('/group/:groupId/monthly', auth, (req, res) => {
  ...
  const rows = db.prepare(`
    ...
    FROM expenses WHERE group_id = ? AND date >= date('now', '-6 months')
    GROUP BY month ORDER BY month ASC
...
router.get('/group/:groupId/members', auth, (req, res) => {
  if (!db.prepare('SELECT 1 FROM group_members WHERE group_id = ? AND user_id = ?').get(req.params.groupId, req.user.id))
    return res.status(403).json({ error: 'Not a member' });
  ...
  const rows = db.prepare(`
    ...
    WHERE gm.group_id = ?
    GROUP BY u.id
  `).all(req.params.groupId, req.params.groupId);
